    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        switch (item.type) {
            ...
            case 'criterion':
                ...
                const sum = nextRelevant.reduce((acc, c) => acc + c.pondere, 0);
                if (sum <= 0) {
                  alert("Pondere totală invalidă pentru criterii.");
                  return;
                }
                let finalRelevant = nextRelevant;
                if (Number(sum.toFixed(4)) !== 1) {
                  const pct = Math.round(sum * 100);
                  const agree = window.confirm(`Suma ponderilor este ${pct}%. Doriți să normalizăm automat la 100%?`);
                  if (!agree) return;
                  finalRelevant = nextRelevant.map(c => ({ ...c, pondere: c.pondere / sum }));
                }
                setCriteria(prev => {
                  const others = prev.filter(c => !(c.etapaId === selectedStageId && c.categorieId === selectedCategoryId));
                  return [...others, ...finalRelevant];
                });
                break;
        }
        onClose();
...
const AuditAndExport: React.FC<AdminViewProps> = (props) => {
    ...
    const exportAssignmentsToCsv = () => {
        const { assignments, candidates, judges, stages, categories } = props;
        ...
        const headers = ['ID','Candidat','Jurat','Etapa','Categorie','Status','Scor Final','Last Modified'];
        const csvRows = [headers.join(',')];
        ...
        assignments.forEach(a => {
            ...
            const row = [
                escapeCsvCell(a.id),
                escapeCsvCell(`${cand?.nume || a.candidatId}`),
                escapeCsvCell(`${judge?.nume || a.juratId}`),
